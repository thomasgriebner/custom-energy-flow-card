...
export function renderBatteryRing(socPct: number, color: string): SVGTemplateResult {
  ...
  const label = svg`<text x="${LABEL_X}" y="${LABEL_Y}" text-anchor="middle" dominant-baseline="middle" font-size="9" font-weight="400" fill="#ffffff" transform="rotate(-45 ${LABEL_X} ${LABEL_Y})" part="battery-ring-label">${formatSocPct(clamped)}</text>`;
