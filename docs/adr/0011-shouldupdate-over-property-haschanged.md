...
HA pusht `hass`-Updates bei jeder State-Änderung *aller* Entities — auch
...
* Beim Hinzufügen einer neuen `@property` muss man bedenken, ob die im
  shouldUpdate-Filter berücksichtigt werden muss. *Mitigation:* Default
...
* Lit-Best-Practice-Doku referenziert primär `hasChanged` für selektive
  ...
  *Mitigation:* dieser ADR + Spec §5.7 + conventions.md erklären den Grund.
...
* ✅ Volle Kontrolle
* ❌ Läuft *nach* willUpdate; Compute läuft ggf. trotzdem
